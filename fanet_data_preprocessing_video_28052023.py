...
def compile_micro_sim_data_v2(file_list):
    ...
    gcs_mon_file = [file for file in file_list if ('_GCS-Wlan' in file)]
    ...
    gw_rx_file = [file for file in file_list if ('_GW-Rx.csv' in file)]
    gw_tx_file = [file for file in file_list if ('_GW-Tx.csv' in file)]
    gw_mon_file = [file for file in file_list if ('_GW-Wlan' in file)]
    gw_pd_file = [file for file in file_list if ('_GW-PacketDrop.csv' in file)]
    ...
    video_df_list = [gw_video_tx_df, gcs_video_rx_df, gw_video_pd_df] # For video packets (NOTE: some packet drops for video recorded in gcs_pd_df)
    ...
    return rx_df_list, tx_df_list, pd_df_list, video_df_list, mon_df_list
...
def process_video_packets(video_tx_df, video_rx_df, gw_video_pd_df, h_dist, height):
    '''
    Date: 30/5/2023
    To process video packets
    NOTE: Although incr rcvd is recorded in gcs_pd_df, but queue overflow and retry limit reach are recorded in gw_video_pd_df, which is what we want
    video_tx_df: Tx DF for video packets (GW-Video-Tx.csv)
    video_rx_df: Rx DF for video packets (GCS-Video-Rx.csv)
    gw_video_pd_df: Packet drop DF from GW for video packets
    Output: rx_df: Modified rx_df containing info on packets from tx_df received and dropped 
    '''
    video_rx_df["U2G_Distance"] = math.sqrt(h_dist**2 + height**2)
    # First, get the list of packets missing from Rx DF but transmitted in Tx DF
    video_seq_rcvd = video_rx_df["Packet_Seq"].values
    tx_df_failed = video_tx_df.loc[~(video_tx_df["Packet_Seq"].isin(video_seq_rcvd))]
    tx_df_failed_dict = tx_df_failed.to_dict('records')
    failed_pkt_list = [] # Using list to store failed packet instead of doing pd.concat for appending every packets. This should be much faster
    # Only iterating through packets that failed to be received
    for row in tqdm(tx_df_failed_dict):
        ...
        packetType = row["Packet_Name"] # In TX CSV files, Packet_Type is called Packet_Name
        packetSeq = row["Packet_Seq"]
        packetName = packetType + "-" + str(packetSeq)
        dest_addr = row["Dest_Addr"]

        pkt_drops = gw_video_pd_df.loc[(gw_video_pd_df["Packet_Type"] == packetType) & (gw_video_pd_df["Packet_Seq"] == packetSeq)] # Packets dropped as recorded in GW-Video-PacketDrop.csv
        if not pkt_drops.empty: # Find the packet drops for this particular packet
            drop_reasons = pkt_drops["Packet_Drop_Reason"].values # List of pkt drop reasons at GW and Rx and Tx
            # Detect the occurences of each failure modes for a particular packet
            incorrect_rcvd = np.count_nonzero(drop_reasons == "INCORRECTLY_RECEIVED")
            queue_overflow = np.count_nonzero(drop_reasons == "QUEUE_OVERFLOW")
            retry_limit_excd = np.count_nonzero(drop_reasons == "RETRY_LIMIT_REACHED")
            interface_down = np.count_nonzero(drop_reasons == "INTERFACE_DOWN")
            ...
            u2g_sinr = pkt_drops["U2G_SINR"].max()
            u2g_ber = pkt_drops["U2G_BER"].max()
            delay = pkt_drops['RxTime']-pkt_drops['TxTime']
            delay = delay.max()
            u2g_distance = pkt_drops["U2G_Distance"].max()
            # Packet State Based on Failure Mode
            if interface_down > 0:
    ...
    failed_pkt_df = pd.DataFrame(failed_pkt_list)
    video_rx_df = pd.concat([video_rx_df,failed_pkt_df], ignore_index = True)
    video_rx_df = video_rx_df.sort_values("RxTime")
    video_rx_df = video_rx_df.reset_index()
    return video_rx_df
...
def process_scenario_v2(scenario, sim_root_path, dl_delay_threshold, ul_delay_threshold, video_delay_threshold, save_path, GX_GCS=500):
    '''
    ...
    rx_df_list, tx_df_list, pd_df_list, video_df_list, mon_df_list = compile_micro_sim_data_v2(scenario_files)
    ...
    for i in range(1, len(video_df_list)): # Only for Rx_DF and PD_DF
        if not video_df_list[i].empty:
            video_df_list[i][["Packet_Type", "Packet_Seq"]] = video_df_list[i].Packet_Name.str.split("-",expand=True)
            video_df_list[i]["Packet_Type"] = pd.Categorical(video_df_list[i]["Packet_Type"].values, categories=pkt_type_cat)
            video_df_list[i]["Packet_Seq"] = video_df_list[i].Packet_Seq.astype("uint32")
        else:
            video_df_list[i]["Packet_Type"] = []
            video_df_list[i]["Packet_Seq"] = []
    ...
    video_rx_df = process_received_packets_v4(video_rx_df, video_delay_threshold)
    video_df = process_video_packets(video_tx_df, video_rx_df, video_pd_df, h_dist=h_dist, height=height)
    if video_df is not None:
        video_df = process_throughput(video_df, 1)
        video_df["Height"] = height
        # video_df["Inter_UAV_Distance"] = inter_uav_distance
        video_df["Num_Members"] = num_member
        video_df["UAV_Sending_Interval"] = uav_sending_interval
        video_df.drop(columns=["Packet_Type", "Packet_Seq"], inplace=True)
        video_df["U2G_H_Dist"] = video_df.apply(h_dist_calc, axis=1)
        video_df[['Mean_SINR',"Std_Dev_SINR"]]= video_df.apply(lambda row: sinr_lognormal_approx(row['U2G_H_Dist'],row['Height']),axis=1,result_type='expand')
        # Drop "index" column if its there
        if "index" in video_df.columns:
            video_df = video_df.drop(["index"], axis=1)
        video_df.to_csv(os.path.join(save_path,"{}_video.csv".format(scenario)), index=False)

def process_sim_data_v2(sim_root_path, dl_delay_threshold, ul_delay_threshold, video_delay_threshold, save_path):
    ...
    with Pool(25) as pool:
        pool.starmap(process_scenario_v2, zip(scenario_list, repeat(sim_root_path), repeat(dl_delay_threshold), repeat(ul_delay_threshold), repeat(video_delay_threshold), repeat(save_path), repeat(GX_GCS)))
...
if __name__ == "__main__":
    ...
    '''
    sim_root_path = "/media/research-student/One Touch/FANET Datasets/Dataset_NP10000_MultiModulation_Hovering_Video/Test/Test_Dataset_1"
    save_path = "/media/research-student/One Touch/FANET Datasets/Dataset_NP10000_MultiModulation_Hovering_Video/Test/Test_Dataset_1_processed"
    # sim_root_path = "/home/rlim0005/FANET_Dataset/Dataset_NP10000_MultiModulation_Hovering_Video/QAM64"
    # save_path = "/home/rlim0005/FANET_Dataset/Dataset_NP10000_MultiModulation_Hovering_Video/QAM64_processed"

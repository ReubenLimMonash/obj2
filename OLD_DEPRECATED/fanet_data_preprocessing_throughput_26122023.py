...
def process_scenario_v4(scenario_path, save_path, dl_stride, ul_stride, dl_slot_size, ul_slot_size, dl_delay_threshold, ul_delay_threshold, mode='timeSlot', vid_stride=1, vid_slot_size=1, vid_delay_threshold=0, GX_Offset=0):
    ...
    for run in runs:
        ...
        num_dl_reliable = np.sum([len(df.loc[df["Delay"]<dl_delay_threshold]) for df in rx_df_list[1:]])
        num_dl_sent = len(gcs_tx_df)
        dl_reliability = num_dl_reliable / num_dl_sent
        num_ul_reliable = len(gcs_rx_df.loc[gcs_rx_df["Delay"]<ul_delay_threshold])
        num_ul_sent = len(uavs_tx_df)
        ul_reliability = num_ul_reliable / num_ul_sent
        num_vid_reliable = len(video_rx_df.loc[video_rx_df["Delay"]<vid_delay_threshold])
        num_vid_sent = len(video_tx_df)
        vid_reliability = num_vid_reliable / num_vid_sent
        metric_list.append({"Run": run_num,"Total_Reliability_DL": dl_reliability, "Total_Reliability_UL": ul_reliability, "Total_Reliability_Vid": vid_reliability})
        run_num = run_num + 1
...
if __name__ == "__main__":
    ...
    '''
    sim_root_path = "/media/research-student/One Touch/FANET_Dataset/DJISpark_Manual_Control_No_Interference/data"
    save_path = "/media/research-student/One Touch/FANET_Dataset/DJISpark_Manual_Control_No_Interference/data_processed"

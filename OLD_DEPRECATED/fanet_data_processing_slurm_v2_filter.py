...
def process_sim_data_v2(sim_root_path, delay_threshold, save_path):
    ...
    scenario_list_filtered = []

    # Filter out the scenarios that has already been processed
    for scenario in scenario_list:
        downlink_file = os.path.join(save_path,"{}_downlink.csv".format(scenario)) 
        uplink_file = os.path.join(save_path,"{}_uplink.csv".format(scenario)) 
        if not (os.path.exists(downlink_file) and os.path.exists(uplink_file)):
            scenario_list_filtered.append(scenario)
    ...
    with Pool(32) as pool:
        pool.starmap(process_scenario_v2, zip(scenario_list_filtered, repeat(sim_root_path), repeat(delay_threshold), repeat(NP), repeat(save_path), repeat(sending_interval_range)))

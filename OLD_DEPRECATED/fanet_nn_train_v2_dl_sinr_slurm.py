...
checkpoint_filepath = '/home/rlim0005/nn_checkpoints/nn_v2_hovering_novideo_sinr_dl'
delay_threshold = 1
...
dl_df.dropna(subset=["Mean_SINR", "Std_Dev_SINR"], inplace=True)

dl_df.sort_values(by = "U2G_H_Dist", inplace=True)

# data_df = dl_df[["Mean_SINR", "Std_Dev_SINR", "Num_Members", "UAV_Sending_Interval", "Incorrectly_Received", "Queue_Overflow"]].copy()
# data_df["Reliable"] = np.where(dl_df['Packet_State'] == "Reliable" , 1, 0)
dl_df["Delay_Exceeded"] = np.where(dl_df['Delay'] > delay_threshold, 1, 0)
...
dl_df["Mean_SINR"] = dl_df["Mean_SINR"].div(max_mean_sinr)
dl_df["Std_Dev_SINR"] = dl_df["Std_Dev_SINR"].div(max_std_dev_sinr)
dl_df["Num_Members"] = dl_df["Num_Members"].div(max_num_members)
dl_df["UAV_Sending_Interval"] = dl_df["UAV_Sending_Interval"].div(max_uav_send_int)
# dl_df["Bytes"] = dl_df["Bytes"].div(max_bytes)

# Let's limit the number of incorrectly rcvd packets to 7, if for some reason it goes beyond that
dl_df.loc[(dl_df["Incorrectly_Received"] > 7), "Incorrectly_Received"] = 7

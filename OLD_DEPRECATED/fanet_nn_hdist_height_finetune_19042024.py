...
Modified: To use hdist and height as inputs rather than mean and std dev SINR
'''
...
def generate_reliability_train_test_dataset(dataset_details_df, test_split=0.2):
    ...
    for row in tqdm(dataset_details_df.itertuples()):
        ...
        if num_reliable > 1:
            reliable_packets = pd.DataFrame({"Horizontal_Distance": hdist, "Height": height, "UAV_Sending_Interval": uav_send_int, "MCS": mcs, "Packet_State": "Reliable"}, index=[0])
            ...
            reliable_packets_train = pd.DataFrame({"Horizontal_Distance": hdist, "Height": height, "UAV_Sending_Interval": uav_send_int, "MCS": mcs, "Packet_State": "Reliable"}, index=[0])
        ...
        if num_delay_excd > 1:
            delay_excd_packets = pd.DataFrame({"Horizontal_Distance": hdist, "Height": height, "UAV_Sending_Interval": uav_send_int, "MCS": mcs, "Packet_State": "Delay_Exceeded"}, index=[0])
            ...
            delay_excd_packets_train = pd.DataFrame({"Horizontal_Distance": hdist, "Height": height, "UAV_Sending_Interval": uav_send_int, "MCS": mcs, "Packet_State": "Delay_Exceeded"}, index=[0])
        ...
        if num_q_overflow > 1:
            q_overflow_packets = pd.DataFrame({"Horizontal_Distance": hdist, "Height": height, "UAV_Sending_Interval": uav_send_int, "MCS": mcs, "Packet_State": "QUEUE_OVERFLOW"}, index=[0])
            ...
            q_overflow_packets_train = pd.DataFrame({"Horizontal_Distance": hdist, "Height": height, "UAV_Sending_Interval": uav_send_int, "MCS": mcs, "Packet_State": "QUEUE_OVERFLOW"}, index=[0])
        ...
        if num_incr_rcvd > 1:
            incr_rcvd_packets = pd.DataFrame({"Horizontal_Distance": hdist, "Height": height, "UAV_Sending_Interval": uav_send_int, "MCS": mcs, "Packet_State": "RETRY_LIMIT_REACHED"}, index=[0])
            ...
            incr_rcvd_packets_train = pd.DataFrame({"Horizontal_Distance": hdist, "Height": height, "UAV_Sending_Interval": uav_send_int, "MCS": mcs, "Packet_State": "RETRY_LIMIT_REACHED"}, index=[0])
...
if __name__ == "__main__":
    ...
    MODEL_FILEPATH = '/home/rlim0005/nn_checkpoints/djispark_nnv4_wobn_vid_hdist_17042024/model.020-0.2658.h5'
    CHECKPOINT_FILEPATH = '/home/rlim0005/nn_checkpoints/parrotar2_nnv4_wobn_finetune_vid_hdist_30042024'
    DATASET_PATH = "/home/rlim0005/FANET_Dataset/Dataset_NP10000_ParrotAR2/ParrotAR2_Video_Reliability.csv"
    ...
    dataset_details_df = normalize_data(dataset_details_df, columns=["Horizontal_Distance", "Height", "UAV_Sending_Interval", "MCS"], save_details_path=CHECKPOINT_FILEPATH) 
    ...
    X_train = df_train[["Horizontal_Distance", "Height", "UAV_Sending_Interval", "MCS"]].values
    X_test = df_test[["Horizontal_Distance", "Height", "UAV_Sending_Interval", "MCS"]].values

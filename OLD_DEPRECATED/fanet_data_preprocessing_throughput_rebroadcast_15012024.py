...
def process_scenario_broadcast(scenario_path, save_path, num_processes, 
    ...
    with Pool(num_processes) as pool:
        for result in pool.starmap(process_run, zip(run_number, repeat(run_file_path), repeat(save_path), repeat(scenario), repeat(height), repeat(uav_sending_interval), repeat(mcs_index), repeat(uav_speed),
            ...
            metric_list.append({"Run": result[0], "Num_Pkts_Reliable_DL": result[3], "Num_Pkts_Reliable_UL": result[4], 
                            ...
                            "Useful_Packet_Reception_Ratio_DL": result[-1],
                            "Total_Reliability_DL": result[-3], "Total_Reliability_UL": result[-2]})
...
def process_run(run_number, file_path, save_path, scenario, height, uav_sending_interval, mcs_index, uav_speed,
                dl_stride, ul_stride, dl_slot_size, ul_slot_size, dl_delay_threshold, ul_delay_threshold):
    '''
    ...
    for i in range(1,len(rx_df_list)):
        ...
        if not uav_rx_df.empty:
            ...
            uav_rx_df = uav_rx_df.loc[(uav_rx_df["Delay"] <= dl_delay_threshold)]
            dl_num_reliable += len(uav_rx_df) # Why not calc this from process_throughput_sliding_window_time_broadcast? Because of possibly overlapping time windows
            uav_throughput_df = process_throughput_sliding_window_time_broadcast(uav_rx_df, gcs_tx_df, dl_slot_size, dl_stride, max_time, dl_delay_threshold, uav_speed)
            if not uav_throughput_df.empty: # uav_throughput_df can still be empty even if there are uav_rx_df, since there could be 0 reliable in uav_rx_df
                uav_throughput_df.loc[uav_throughput_df.index[0], 'Height'] = height
                uav_throughput_df.loc[uav_throughput_df.index[0], 'UAV_Sending_Interval'] = uav_sending_interval
                uav_throughput_df.loc[uav_throughput_df.index[0], 'MCS_Index'] = mcs_index
                uav_throughput_df.loc[uav_throughput_df.index[0], 'UAV_Speed'] = uav_speed
            else:
    ...
    uavs_pd_df = uavs_pd_df.loc[['CNCData' not in name for name in uavs_pd_df.Packet_Name]].copy() # Remove packets broadcasted by GCS in uavs_pd_df
    if not gcs_rx_df.empty:
        ...
        gcs_rx_df = gcs_rx_df.loc[(gcs_rx_df["Delay"] <= ul_delay_threshold)]
        ...
        ul_throughput_df.loc[ul_throughput_df.index[0], 'MCS_Index'] = mcs_index
    ...
    return (run_number, num_broadcasts_dl, num_broadcasts_ul, dl_num_reliable, num_reliable_pkts_ul, total_reliability_dl, total_reliability_ul, uprr_dl)
...
if __name__ == "__main__":
    ...
    '''
    # sim_root_path = "/media/research-student/One Touch/FANET_Dataset/DJISpark_Obj3_Retransmission_Datasets/DJISpark_ReBroadcast_Protocols_No_Interference/throughput_based_case6_MAW5"
    # save_path = "/media/research-student/One Touch/FANET_Dataset/DJISpark_Obj3_Retransmission_Datasets/DJISpark_ReBroadcast_Protocols_No_Interference/throughput_based_case6_MAW5_processed"
    sim_root_path = "/home/wlau0003/Reuben_ws/FANET_Dataset/DJISpark_Retransmission_Protocols_MANET_Interference/fixed_prob_default_prob100"
    save_path = "/home/wlau0003/Reuben_ws/FANET_Dataset/DJISpark_Retransmission_Protocols_MANET_Interference/fixed_prob_default_prob100_processed"
    ...
    for scenario_path in scenario_list:
         process_scenario_broadcast(scenario_path, save_path, num_processes, dl_stride, ul_stride, dl_slot_size, ul_slot_size, dl_delay_threshold, ul_delay_threshold)

...
def process_throughput_sliding_window_time_broadcast(rx_df, tx_df, winSize, stride, max_time, delay_threshold, uav_speed):
    ...
    Modified: To handle broadcast case. Measured throughput is calc without considering duplicated transmission.
    ...
    for i in range(math.ceil((max_time - winSize) / stride)):
        ...
        if not (rx_df_in_range.empty):
            ...
            totalBytes = df_reliable["Bytes"].sum()
            throughput = totalBytes / winSize
            pkt_outstanding = pkt_outstanding[~pkt_outstanding.Packet_Name.isin(df_reliable.Packet_Name)] # Remove packets that have been reliably received in each time window
...
def process_scenario_v4(scenario_path, save_path, dl_stride, ul_stride, dl_slot_size, ul_slot_size, dl_delay_threshold, ul_delay_threshold, mode='timeSlot', vid_stride=1, vid_slot_size=1, vid_delay_threshold=0, GX_Offset=0):
    ...
    for r in range(len(runs)):
        ...
        gcs_rx_df = gcs_rx_df.loc[['CNCData' not in name for name in gcs_rx_df.Packet_Name]] # Remove packets broadcasted by GCS in gcs_rx_df
        ...
        num_reliable_pkts_dl = np.sum([df["Num_Reliable"].sum() for df in uav_throughput_df_list])
        num_fail_pkts_dl = np.sum([df["Num_Fail"].sum() for df in uav_throughput_df_list])
        total_reliability_dl = num_reliable_pkts_dl / (num_reliable_pkts_dl+num_fail_pkts_dl)
        ...
        total_reliability_ul = len(gcs_rx_df_reliable) / (len(uavs_tx_df))
        metric_list.append({"Run": r, "Num_Broadcast_DL": num_broadcasts_dl, "Num_Broadcast_UL": num_broadcasts_ul,
...
if __name__ == "__main__":
    ...
    '''
    # sim_root_path = "/media/research-student/One Touch/FANET_Dataset/DJISpark_Measured_Throughput_10000Samples/data_test"
    # save_path = "/media/research-student/One Touch/FANET_Dataset/DJISpark_Measured_Throughput_10000Samples/data_test_processed"
    sim_root_path = "/media/research-student/One Touch/FANET_Dataset/DJISpark_ReBroadcast_Protocols_Test/measured_throughput_test"
    save_path = "/media/research-student/One Touch/FANET_Dataset/DJISpark_ReBroadcast_Protocols_Test/measured_throughput_test_processed"

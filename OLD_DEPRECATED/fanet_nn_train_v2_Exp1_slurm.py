...
Modified: Train for experiment 1 only, with higher proportion of data at the "waterfall"
'''
...
checkpoint_filepath = '/home/rlim0005/nn_checkpoints/nn_v2_Exp1'
...
# Make data at 100m - 250m have higher proportion
dl_df_in = dl_df.loc[(dl_df["U2G_H_Dist"] >= 100) & (dl_df["U2G_H_Dist"] < 250)].copy()
dl_df_out = dl_df.loc[(dl_df["U2G_H_Dist"] < 100) | (dl_df["U2G_H_Dist"] >= 250)].copy()
dl_df_out = dl_df_out.iloc[3::5, :].copy()
dl_df = pd.concat([dl_df_in, dl_df_out])
...
# Use all data fro training and testing
X_train = data_df[["U2G_H_Dist", "Height", "Num_Members", "Bytes", "Sending_Interval"]].values
...
with open(os.path.join(checkpoint_filepath, 'trainHistoryDict_27032023'), 'wb') as file_pi:

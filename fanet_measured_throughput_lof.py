# Date: 27/10/2023
# fanet__measured_throughput_lof.ipynb
# Desc: To train novelty detection model for measured throughput using LOF model
...
def normalize_data(df_in, columns, link, save_details_path=None):
    ...
    if "Height" in columns:
        df["Height"] = df["Height"].apply(lambda x: 20*(x-min_height)/(max_height-min_height) - 10)
    ...
    if "UAV_Sending_Interval" in columns:
        df["UAV_Sending_Interval"] = df["UAV_Sending_Interval"].replace({10:-10, 20:-5, 40:0, 66.7: 5, 100:10, 1000:20})
    ...
    if "Modulation" in columns:
        df['Modulation'] = df['Modulation'].replace({"BPSK":10, "QPSK":3.333, 16:-3.333, "QAM-16":-3.333, "QAM16":-3.333, 64:-1, "QAM-64":-10, "QAM64":-10})
    if "MCS" in columns:
        df["MCS"] = df["MCS"].apply(lambda x: 20*(x-min_mcs)/(max_mcs-min_mcs) - 10)
    if "MCS_Index" in columns:
        df["MCS_Index"] = df["MCS_Index"].apply(lambda x: 20*(x-min_mcs)/(max_mcs-min_mcs) - 10)
    if "UAV_Speed" in columns:
        df["UAV_Speed"] = df["UAV_Speed"].apply(lambda x: 20*(x-min_uav_speed)/(max_uav_speed-min_uav_speed) - 10)
...
if __name__ == "__main__":
    ...
    SAVE_FILEPATH = '/home/wlau0003/Reuben_ws/lof_checkpoints/djispark_throughput_lof_dl.joblib' 
    LINK = "Downlink"
    ...
    # Train LOF model
    model_1 = LocalOutlierFactor(n_neighbors=100, novelty=True, n_jobs=64)
    model_1.fit(X_train)
    # Save model
    dump(model_1, SAVE_FILEPATH) 
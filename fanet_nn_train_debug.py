...
if __name__ == "__main__":
    ...
    # DATASET_PATH = "/home/research-student/omnetpp_sim_results/PCAP_Test/ParrotAR2_ConstantSI/BPSK_Test.csv"
    ...
    #                 dtype=df_dtypes)
    # df["Modulation"] = "BPSK"
    # df_train = normalize_data(df.copy(), columns=["Mean_SINR", "Std_Dev_SINR", "UAV_Sending_Interval", "Modulation", "Packet_State"], save_details_path=None)
    # df_holdout = normalize_data(df.copy(), columns=["Mean_SINR", "Std_Dev_SINR", "UAV_Sending_Interval", "Modulation", "Packet_State"], save_details_path=None)
    ...
    # # data_df = df = pd.DataFrame(np.hstack((X_train,packet_state_train)), columns=["Mean_SINR", "Std_Dev_SINR", "UAV_Sending_Interval", "Modulation", "Reliable", "Q_Overflow", "Incr_Rcvd", "Delay_Excd"])
    # # data_df.to_csv("BPSK_Test.csv")
    ...
    train_data_generator = DataGenerator(dataset_details, test_split=0, type='train', batch_size=BATCHSIZE, shuffle=None)
    val_data_generator = DataGenerator(dataset_details, test_split=0, type='test', batch_size=BATCHSIZE, shuffle=None)
    ...
    # CHECKPOINT_FILEPATH = '/home/research-student/omnetpp_sim_results/PCAP_Test/ParrotAR2_ConstantSI'
    # # DATASET_PATH = "/home/research-student/omnetpp_sim_results/PCAP_Test/DJISpark_ConstantSI/DJISpark_ConstantSI_Downlink_Reliability.csv"
    # DATASET_PATH = "/home/research-student/omnetpp_sim_results/PCAP_Test/ParrotAR2_ConstantSI/test_recon_training.csv"

    # # Load dataset =================================================
    # df_dtypes = {"TxTime": np.float32, "U2G_Distance": np.float32, "Height": np.int16,	"Num_Members": np.int16, "UAV_Sending_Interval": np.float64, "Bytes": np.int16, 
    #         "U2G_SINR": np.float32, "U2G_BER": np.float32, "Delay": np.float32, "Throughput": np.float32, "Queueing_Time": np.float32, "Packet_State": 'category', 
    #         "Retry_Count": np.int8, "Incorrectly_Received": np.int8, "Queue_Overflow": np.int8, "Packet_Name": 'string', "Mean_SINR": np.float32, "Std_Dev_SINR": np.float32,}
    # df = pd.read_csv(DATASET_PATH,
    #                 usecols = ["Mean_SINR", "Std_Dev_SINR", "UAV_Sending_Interval", "Modulation", "Num_Reliable", "Num_Delay_Excd", "Num_Incr_Rcvd", "Num_Q_Overflow"],
    #                 dtype=df_dtypes)
    # # df_train = normalize_data(df.copy(), columns=["Mean_SINR", "Std_Dev_SINR", "UAV_Sending_Interval", "Modulation", "Packet_State"], save_details_path=None)
    # # df_holdout = normalize_data(df.copy(), columns=["Mean_SINR", "Std_Dev_SINR", "UAV_Sending_Interval", "Modulation", "Packet_State"], save_details_path=None)
    # X_train = df[["Mean_SINR", "Std_Dev_SINR", "UAV_Sending_Interval", "Modulation"]].values
    # packet_state_train = df[["Num_Reliable", "Num_Delay_Excd", "Num_Incr_Rcvd", "Num_Q_Overflow"]].values
    ...
    # date = datetime.now()
    # date_str = date.strftime("%d%m%Y")
    # history = model.fit(X_train, packet_state_train, epochs=EPOCHS, batch_size=320, callbacks=[model_checkpoint_callback, ClearMemory()], validation_data=(X_train, packet_state_train))
    # # history = model.fit(train_data_generator, 
    # #                     epochs=EPOCHS,
    # #                     # callbacks=[model_checkpoint_callback, ClearMemory()],
    # #                     callbacks=[model_checkpoint_callback],

"""
Date: 18/11/2025
Desc: Reliability-prediction-based MCS adaptation using NN models.
...
def mdp_nn(dl_model, ul_model, vid_model, height, usi, reliability_th=0.99, init_hdist=0, dmax=500, hdist_step_size=5, save_file=None):
    '''
    ...
    init_mcs_dl_rel = np.array([probs[0] for probs in dl_probs])
    ul_probs = ul_model.predict(inputs, verbose=0)
    init_mcs_ul_rel = np.array([probs[0] for probs in ul_probs])
    vid_probs = vid_model.predict(inputs, verbose=0)
    init_mcs_vid_rel = np.array([probs[0] for probs in vid_probs])
    ...
    for init_mcs in possible_init_mcs_indexes: # For each possible initial MCS that fulfills reliability requirement, run the MDP
        ...
        while True: 
            ...
            dl_probs = dl_model.predict([[mean_sinr, std_dev_sinr, uav_send_int_norm[usi], norm_MCS(curr_MCS)]], verbose=0)
            ul_probs = ul_model.predict([[mean_sinr, std_dev_sinr, uav_send_int_norm[usi], norm_MCS(curr_MCS)]], verbose=0)
            vid_probs = vid_model.predict([[mean_sinr, std_dev_sinr, uav_send_int_norm[usi], norm_MCS(curr_MCS)]], verbose=0)
            ...
            if not next_mcs_rel_check:
                ...
                dl_probs = dl_model.predict(inputs, verbose=0)
                next_mcs_dl_rel = np.array([probs[0] for probs in dl_probs])
                ul_probs = ul_model.predict(inputs, verbose=0)
                next_mcs_ul_rel = np.array([probs[0] for probs in ul_probs])
                vid_probs = vid_model.predict(inputs, verbose=0)
                next_mcs_vid_rel = np.array([probs[0] for probs in vid_probs])
...
def run_mdp(height, usi, max_hdist_df, hdist_step_size, dl_model_path, ul_model_path, vid_model_path, reliability_th, save_path):
    # Load models
    ...
    if max_hdist > 0: # Exclude test cases with 0 max hdist 
        ...
        mdp_nn(dl_model, ul_model, vid_model, height, usi, reliability_th=reliability_th, init_hdist=0, dmax=max_hdist, hdist_step_size=hdist_step_size, save_file=save_file)
...
if __name__ == "__main__":
    ...
    SAVE_PATH = "/home/research-student/omnet-fanet/data-processing-scripts/journal_2_scripts/predictions_mcs_adaptation/predictions_5m_nn_{}".format(RELIABILITY_TH_STR)
    NUM_WORKERS = 32

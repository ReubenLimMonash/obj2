...
Desc: Pandas data processing of FANET dataset with multiprocessing.
      ...
      Modified to handle files for each scenarios being in separate folders
'''
...
def compile_micro_sim_data_v2(file_list, video=False):
    ...
    gcs_mon_file = [file for file in file_list if ('GCS-Wlan.csv' in file)]
    gcs_pd_file = [file for file in file_list if ('GCS-PacketDrop.csv' in file)]
    ...
    gw_mon_file = [file for file in file_list if (('GW-Wlan' in file))]
    gw_pd_file = [file for file in file_list if (('GW-PacketDrop' in file))]
    if len(gw_rx_file) > 0:
    ...
    uav_mon_files = [file for file in file_list if (('UAV-' in file) and ('Wlan' in file))]
    uav_pd_files = [file for file in file_list if (('UAV-' in file) and ('PacketDrop' in file))]
    uav_rx_files.sort()
    uav_tx_files.sort()
    uav_mon_files.sort()
    uav_pd_files.sort()
    ...
    if video:
        ...
        gw_video_pd_file = [file for file in file_list if ('GW-Video-PacketDrop.csv' in file)]
...
def process_scenario_v3(scenario_path, dl_delay_threshold, ul_delay_threshold, vid_delay_threshold=0, GX_Offset=0):
    '''
    ...
    h_dist = float([param for param in scenario_params if ("Distance" in param)][0].split('-')[-1]) - GX_Offset 
    ...
    dl_result = {"Horizontal_Distance": h_dist, "Height": height, "UAV_Sending_Interval": uav_sending_interval, "Modulation": modulation, "Bitrate": bitrate,
                 "Mean_SINR": mean_sinr, "Std_Dev_SINR": std_dev_sinr, 
...
def process_sim_data_v3(sim_root_path, dl_delay_threshold, ul_delay_threshold, vid_delay_threshold, save_path, num_workers):
    ...
    with Pool(num_workers) as pool:
        for result in pool.starmap(process_scenario_v3, zip(scenario_list, repeat(dl_delay_threshold), repeat(ul_delay_threshold), repeat(vid_delay_threshold), repeat(GX_GCS))):
...
if __name__ == "__main__":
    ...
    '''
    sim_root_path = "/home/research-student/FANET_Dataset/Dataset_NP100000_DJISpark/mdp_test_data"
    save_path = "/home/research-student/FANET_Dataset/Dataset_NP100000_DJISpark/mdp_test_data_processed"
    # sim_root_path = "/home/clow0003/Reuben_ws/FANET_Dataset/usi_adaptation_max_hdist/data"
    # save_path = "/home/clow0003/Reuben_ws/FANET_Dataset/usi_adaptation_max_hdist/data_processed_batch1"
    # sim_root_path = "/home/rlim0005/FANET_Dataset/Dataset_NP100000_DJISpark/data"
    # save_path = "/home/rlim0005/FANET_Dataset/Dataset_NP100000_DJISpark/data_height60_processed"
    num_workers = 32

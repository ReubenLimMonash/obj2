...
def mdp_write_actions_2_xml(mdp_actions, nodes, uav_speed, xml_file_path,
                            adapt_height=False, adapt_usi=False, adapt_mcs=True):
    """
    ...
    mdp_actions.drop_duplicates(subset=["Height", "UAV_Send_Interval", "MCS"], keep='first', inplace=True)
    mdp_actions_diff = mdp_actions.diff()
    ...
    first_mdp_actions = mdp_actions.iloc[0]
    ...
    for i in range(1,len(mdp_actions_diff)):
        row_mdp_actions = mdp_actions.iloc[i]
        ...
        if (mdp_actions_diff.iloc[i].MCS != 0.0) and adapt_mcs:
            for node in nodes:
                set_param = ET.SubElement(at, 'set-param', attrib={"module":"{}.wlan[0].mac.dcf.rateControl".format(node), "par":"MCSIndex", "value":str(mcs)})
        # If sendInterval changed, write new sendInterval to scenario script
        if (mdp_actions_diff.iloc[i].UAV_Send_Interval != 0.0) and adapt_usi:
            for node in uav_nodes:
                set_param = ET.SubElement(at, 'set-param', attrib={"module":"{}.app[1]".format(node), "par":"sendInterval", "value":str(usi)+"ms"})
...
def run_mdp(uav_speed, height, mcs, init_usi, usi_list, max_hdist_df, hdist_step_size, nodes, 
            dl_model_path, ul_model_path, vid_model_path, reliability_th, scenario_script_path):
    # Load models
    dl_model = load_model(dl_model_path)
    ul_model = load_model(ul_model_path)
    vid_model = load_model(vid_model_path)
    # Run MDP
    print("UAV Speed:{}, Height: {}, MCS: {}".format(uav_speed, height, mcs))
    max_hdist = max_hdist_df.loc[(max_hdist_df["Height"]==height) & (max_hdist_df["MCS"]==mcs)]["Max_Horizontal_Distance"].values[0]
    if (not pd.isna(max_hdist)) & (max_hdist > 0): # Exclude test cases with 0 max hdist
        ...
        mdp_actions = mdp_nn(dl_model, ul_model, vid_model, reliability_th=reliability_th,
                            horizontal_dist=hdist, heights=[height], uav_send_ints=usi_list, mcs_indexes=[mcs], 
                            init_height=height, init_usi=init_usi, init_mcs_index=mcs, save_file=None, end_at_max=False)
        # Write scenario script
        scenario_script_file = os.path.join(scenario_script_path, 'height-{}_mcs-{}_uavspeed-{}_scenario_script.xml'.format(height, mcs, uav_speed))
        mdp_write_actions_2_xml(mdp_actions, nodes, uav_speed, scenario_script_file, adapt_height=False, adapt_usi=True, adapt_mcs=False)
        print("Done - UAV Speed:{}, Height: {}, MCS: {}".format(uav_speed, height, mcs))
    else:
        print("Test Case with No HDist - UAV Speed:{}, Height: {}, MCS: {}".format(uav_speed, height, mcs))
    return
...
if __name__ == "__main__":
    ...
    MAX_HDIST_DF_PATH = "/home/research-student/omnet-fanet/data-processing-scripts/mdp_max_hdist/MDP_USI_Max_HDist_fm_Sim_90.csv"
    SCENARIO_SCRIPT_PATH = "/home/research-student/omnet-fanet/scenario_scripts_usi_adaptation/scenario_scripts_1m_nn_90_v2"
    # DL_NN_PATH = "/home/rlim0005/FANET_Dataset/Dataset_NP100000_DJISpark/nn_ckpts/model_Downlink.round-1_split-9_0.2027.h5"
    # UL_NN_PATH = "/home/rlim0005/FANET_Dataset/Dataset_NP100000_DJISpark/nn_ckpts/model_Uplink.round-1_split-9_0.1363.h5"
    # VID_NN_PATH = "/home/rlim0005/FANET_Dataset/Dataset_NP100000_DJISpark/nn_ckpts/model_Video.round-1_split-9_0.2720.h5"
    # MAX_HDIST_DF_PATH = "/home/rlim0005/mdp_max_hdist/MDP_USI_Max_HDist_fm_Sim_90.csv"
    # SCENARIO_SCRIPT_PATH = "/home/rlim0005/scenario_scripts_usi_adaptation/scenario_scripts_1m_nn_90_v2"
    RELIABILITY_TH = 0.9
    NUM_WORKERS = 32
    ...
    var_params = [i for i in product(UAV_SPEEDS, HEIGHTS, MCS_LIST)]
    with Pool(NUM_WORKERS) as pool:
        pool.starmap(run_mdp, zip(*zip(*var_params), repeat(INIT_USI), repeat(USI_LIST), repeat(max_hdist_df), repeat(HDIST_STEP_SIZE), repeat(NODES), 
                    repeat(DL_NN_PATH), repeat(UL_NN_PATH), repeat(VID_NN_PATH), repeat(RELIABILITY_TH), repeat(SCENARIO_SCRIPT_PATH)))
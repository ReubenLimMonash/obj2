"""
Date: 20/10/2025
Desc: To perform 5-Fold cross validation on the FANET NN and Cal NN reliability model
...
def dataset_details_train_test_split(dataset_details_df, num_split):
    '''
    ...
    for row in tqdm(dataset_details_df.itertuples()):
        ...
        num_sent_splits = distribute_evenly(num_split, num_sent)
        ...
        num_incr_rcvd_splits = distribute_evenly(num_split, num_incr_rcvd)
        num_q_overflow_splits = distribute_evenly(num_split, num_q_overflow)

        for i in range(num_split):
            dataset_details_splits[i].append({"Horizontal_Distance": horizontal_distance, "Modulation": modulation, "Bitrate": bitrate, "Mean_SINR": mean_sinr, "Std_Dev_SINR": std_dev_sinr, "UAV_Sending_Interval": uav_send_int,
                                  "Num_Sent": num_sent_splits[i], "Num_Reliable": num_reliable_splits[i], "Num_Delay_Excd": num_delay_excd_splits[i], "Num_Incr_Rcvd": num_incr_rcvd_splits[i], "Num_Q_Overflow": num_q_overflow_splits[i]})
...
def generate_reliability_train_test_dataset(dataset_details_df, test_split=0.2):
    ...
    for row in tqdm(dataset_details_df.itertuples()):
        ...
        if num_incr_rcvd > 1:
            ...
            incr_rcvd_packets_test = pd.DataFrame({})
        df_train_concat = pd.concat([reliable_packets_train, delay_excd_packets_train, q_overflow_packets_train, incr_rcvd_packets_train])
        ...
        df_train_concat["Packet_State"] = df_train_concat["Packet_State"].astype("category")
        df_test_concat = pd.concat([reliable_packets_test, delay_excd_packets_test, q_overflow_packets_test, incr_rcvd_packets_test])
        ...
        df_test_concat["Packet_State"] = df_test_concat["Packet_State"].astype("category")
    ...
    df_train["Packet_State"] = df_train["Packet_State"].astype("category")
    ...
    df_test["Packet_State"] = df_test["Packet_State"].astype("category")
...
if __name__ == "__main__":
    # Training params
    K_FOLD = 5
    ROUNDS = 2 # Instead of Epochs, specify the number of rounds to train through all splits of dataset details
    BATCHSIZE = 64
    NUM_SPLIT = 10 # Number of splits for dataset details (This split is because we can't load the entier data into memory, NOTE it is not the k-fold cross validation splits)
    LR = 0.001 # Adam Learning Rate
    L1_REG = 0.001 # L1 Norm Weight Regularization
    LINK = "Downlink" # "Downlink" or "Uplink" or "Video"
    CHECKPOINT_FILEPATH = '/media/research-student/KingstonSSD/FANET_Dataset/Dataset_NP100000_DJISpark/kfold_nn_ckpts'
    DATASET_PATH = "/media/research-student/KingstonSSD/FANET_Dataset/Dataset_NP100000_DJISpark/data_processed/{}_Reliability.csv"
    KFOLD_DATASET_PATH = "/media/research-student/KingstonSSD/FANET_Dataset/Dataset_NP100000_DJISpark/data_processed_kfold/{}_Reliability_kfold_{}_{}.csv"
    ...
    dataset_details_df = pd.read_csv(DATASET_PATH.format(LINK), 
                                ...
                                dtype=df_dtypes)
    dataset_details_df = dataset_details_df.loc[dataset_details_df["Horizontal_Distance"] <= 700] # Only consider Horizontal_Distance <= 700

    # Split the dataset for cross validation NOTE: COMMENT IF ALREADY PERFORMED BEFORE
    kf = KFold(n_splits=K_FOLD, shuffle=True, random_state=42)
    dataset_details_df_kfolds = []
    i = 0
    for train_index, test_index in kf.split(dataset_details_df):
        train_df = dataset_details_df.iloc[train_index]
        test_df = dataset_details_df.iloc[test_index]
        train_df.to_csv(KFOLD_DATASET_PATH.format(LINK, i, "train"), index=False)
        test_df.to_csv(KFOLD_DATASET_PATH.format(LINK, i, "test"), index=False)
        i += 1
    
    # Train the NN Model for each k-fold split
    for K in range(3, K_FOLD): 
        print("==================== Starting K-Fold Split: {} ====================".format(K))
        # Load dataset for this k-fold split
        dataset_details_df = pd.read_csv(KFOLD_DATASET_PATH.format(LINK, K, "train"), 
                                    usecols = ["Horizontal_Distance", "Height", "Mean_SINR", "Std_Dev_SINR", "UAV_Sending_Interval", "Modulation", "Bitrate", 
                                               "Num_Sent", "Num_Reliable", "Num_Delay_Excd", "Num_Incr_Rcvd", "Num_Q_Overflow"],
                                    dtype=df_dtypes)
        # Further split dataset details into NUM_SPLIT splits to reduce memory usage
        dataset_details_df_splits = dataset_details_train_test_split(dataset_details_df, NUM_SPLIT)
        ...
        for j in range(ROUNDS):
            for i in range(NUM_SPLIT):
                ...
                dataset_split = normalize_data(dataset_split, columns=["Mean_SINR", "Std_Dev_SINR", "UAV_Sending_Interval", "MCS"], save_details_path=CHECKPOINT_FILEPATH) 
                ...
                X_train = df_train[["Mean_SINR", "Std_Dev_SINR", "UAV_Sending_Interval", "MCS"]].to_numpy(dtype=np.float32)
                X_test = df_test[["Mean_SINR", "Std_Dev_SINR", "UAV_Sending_Interval", "MCS"]].to_numpy(dtype=np.float32)
                ...
                model_checkpoint_callback = keras.callbacks.ModelCheckpoint(
                    filepath=os.path.join(CHECKPOINT_FILEPATH,"model_{}.kfold-{}.round-{}_split-{}_{{val_loss:.4f}}.h5".format(LINK,K,j,i)),
                    save_weights_only=False,
                    monitor='val_loss',
                    mode='auto',
                    save_freq='epoch')
                history = model.fit(X_train, packet_state_train, epochs=1, batch_size=BATCHSIZE, callbacks=[model_checkpoint_callback, ClearMemory()], validation_data=(X_test, packet_state_test))
                # Save training history
                with open(os.path.join(CHECKPOINT_FILEPATH, '{}_trainHistoryDict.kfold-{}.round-{}_split-{}'.format(LINK,K,j,i)), 'wb') as file_pi:
                    pickle.dump(history.history, file_pi)

        # Save final model
        model.save(os.path.join(CHECKPOINT_FILEPATH,"{}_kfold_{}_final_model.h5".format(LINK, K)))

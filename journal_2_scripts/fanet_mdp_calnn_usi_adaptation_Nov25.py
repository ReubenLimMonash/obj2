...
def mdp_nn(dl_model, ul_model, vid_model, dl_T, ul_T, vid_T, height, mcs, reliability_th=0.99, init_hdist=0, dmax=500, hdist_step_size=5, save_file=None):
    '''
    ...
    for init_usi in possible_init_usi: # For each possible initial USI that fulfills reliability requirement, run the MDP
        ...
        while True: 
            ...
            if not next_usi_rel_check:
                ...
                dl_logits = dl_model.predict(inputs, verbose=0)
                next_usi_dl_rel = np.array([activations.softmax(K.constant([logits/dl_T]), axis=-1)[0][0].numpy() for logits in dl_logits])
                ul_logits = ul_model.predict(inputs, verbose=0)
                next_usi_ul_rel = np.array([activations.softmax(K.constant([logits/ul_T]), axis=-1)[0][0].numpy() for logits in ul_logits])
                vid_logits = vid_model.predict(inputs, verbose=0)
                next_usi_vid_rel = np.array([activations.softmax(K.constant([logits/vid_T]), axis=-1)[0][0].numpy() for logits in vid_logits])  
...
def run_mdp(height, mcs, max_hdist_df, hdist_step_size, dl_model_path, ul_model_path, vid_model_path, 
    ...
    if max_hdist > 0: # Exclude test cases with 0 max hdist 
        ...
        mdp_nn(dl_model, ul_model, vid_model, dl_T, ul_T, vid_T, height, mcs, reliability_th=reliability_th, init_hdist=0, dmax=max_hdist, hdist_step_size=hdist_step_size, save_file=save_file)

"""
Date: 30/10/2025
Desc: Reliability-prediction-based MCS+USI adaptation using NN models.
...
def load_model(model_path):
    ...
    # compile with expected names (kept consistent with existing scripts)
    try:
        model.compile(optimizer='adam', loss={'packet_state': 'categorical_crossentropy'}, metrics={'packet_state': 'accuracy'})
    except Exception:
...
def mdp_nn_both(dl_model, ul_model, vid_model, height, reliability_th=0.99, init_hdist=0, dmax=500, hdist_step_size=5, save_file=None):
    """
    ...
    # Get initial MCS+USI that fulfill reliability requirement at starting point
    ...
    # Get all combinations of mcs_indexes and usi_list
    ...
    init_mcs_usi_dl_rel = np.array([logits[0] for logits in dl_logits])
    ul_logits = ul_model.predict(inputs, verbose=0)
    init_mcs_usi_ul_rel = np.array([logits[0] for logits in ul_logits])
    vid_logits = vid_model.predict(inputs, verbose=0)
    init_mcs_usi_vid_rel = np.array([logits[0] for logits in vid_logits])
    ...
    for init_mcs, init_usi in zip(possible_init_mcs_indexes, possible_init_usi_indexes): # For each possible initial MCS and initial USI combo that fulfills reliability requirement, run the MDP
        ...
        while True: 
            ...
            next_dl_rel = dl_logits[0][0]
            next_ul_rel = ul_logits[0][0]
            next_vid_rel = vid_logits[0][0]
            state_record.append({"Horizontal_Distance": current_hdist, "Height": height, "UAV_Send_Interval": curr_usi, "MCS": curr_MCS, 
            ...
            if not next_mcs_usi_rel_check:
                ...
                dl_logits = dl_model.predict(inputs, verbose=0)
                next_combo_dl_rel = np.array([logits[0] for logits in dl_logits])
                ul_logits = ul_model.predict(inputs, verbose=0)
                next_combo_ul_rel = np.array([logits[0] for logits in ul_logits])
                vid_logits = vid_model.predict(inputs, verbose=0)
                next_combo_vid_rel = np.array([logits[0] for logits in vid_logits])
                ...
                if np.any(next_combo_rel_check):
                    # Get all indexes of combo that fulfill reliability requirement
                    reliable_combo_indexes = np.where(next_combo_rel_check == 1)[0]
                    # Choose the combo that gives the highest mean reliability
...
def run_mdp(height, max_hdist_df, hdist_step_size, dl_model_path, ul_model_path, vid_model_path, reliability_th, save_path):
    ...
    if max_hdist > 0:
        ...
        mdp_nn_both(dl_model, ul_model, vid_model, height, reliability_th=reliability_th, init_hdist=0, dmax=max_hdist, hdist_step_size=hdist_step_size, save_file=save_file)
...
if __name__ == "__main__":
    ...
    SAVE_PATH = "/home/research-student/omnet-fanet/data-processing-scripts/journal_2_scripts/predictions_mcs_n_usi_adaptation/predictions_5m_nn_{}".format(RELIABILITY_TH_STR)
    NUM_WORKERS = 32
    HDIST_STEP_SIZE = 5
    ...
    with Pool(NUM_WORKERS) as pool:
        pool.starmap(run_mdp, [(h, max_hdist_df, HDIST_STEP_SIZE, DL_NN_PATH, UL_NN_PATH, VID_NN_PATH, RELIABILITY_TH, SAVE_PATH) for h in HEIGHTS])
...
Modified: To record every prediction result and not just when MCS changes
...
def mdp_nn(dl_model, ul_model, vid_model, height, usi, reliability_th=0.99, horizontal_dist = None, save_file=None):
    '''
    ...
    mcs_indexes = [0,1,2,3,4,5,6,7] #MCS Indexes, where 0 is lowest MCS (BPSK 1/2) and 7 is highest MCS (64QAM 3/4)
    ...
    # Get the initial MCS from DL, UL and VID models
    # NOTE: UNCOMMENT the below to use NN models to get initial MCS
    ...
    #                     np.array([uav_send_int_norm[usi]]*8).reshape(-1,1), np.array([norm_MCS(mcs) for mcs in mcs_indexes]).reshape(-1,1)))
    # init_mcs_dl_results = dl_model.predict(inputs, verbose=0)
    # init_mcs_dl_rel = [prob[0] for prob in init_mcs_dl_results]
    # init_mcs_ul_results = ul_model.predict(inputs, verbose=0)
    # init_mcs_ul_rel = [prob[0] for prob in init_mcs_ul_results]
    # init_mcs_vid_results = vid_model.predict(inputs, verbose=0)
    # init_mcs_vid_rel = [prob[0] for prob in init_mcs_vid_results]
    ...
    # curr_MCS_index = find_last_occurrence_manual(init_mcs_rel_check, 1)
    # curr_MCS = mcs_indexes[curr_MCS_index]
    # # Record the start state, since it is based on prediction
    # state_record.append({"Horizontal_Distance": horizontal_dist[0], "Height": height, "UAV_Send_Interval": usi, "MCS": curr_MCS, 
    #                     "Status": "Start", # Status is to indicate whether the scheme is still running (Continue) or has ended (Dmax/Abort)
    #                     "DL_Reliability": init_mcs_dl_rel[curr_MCS_index], "UL_Reliability": init_mcs_ul_rel[curr_MCS_index], "Vid_Reliability": init_mcs_vid_rel[curr_MCS_index]})
    ...
    for j in range(1, len(horizontal_dist)): # Traverse the h_dist
        ...
        while (next_dl_rel < reliability_th) or (next_ul_rel < reliability_th) or (next_vid_rel < reliability_th):
            ...
            mean_sinr = 2*(10*math.log10(m)-min_mean_sinr)/(max_mean_sinr-min_mean_sinr) - 1
            std_dev_sinr = 2*(10*math.log10(s)-min_std_dev_sinr)/(max_std_dev_sinr-min_std_dev_sinr) - 1
            next_dl_rel = dl_model.predict([[mean_sinr, std_dev_sinr, uav_send_int_norm[usi], norm_MCS(curr_MCS)]], verbose=0)[0][0]
            next_ul_rel = ul_model.predict([[mean_sinr, std_dev_sinr, uav_send_int_norm[usi], norm_MCS(curr_MCS)]], verbose=0)[0][0]
            next_vid_rel = vid_model.predict([[mean_sinr, std_dev_sinr, uav_send_int_norm[usi], norm_MCS(curr_MCS)]], verbose=0)[0][0]
            state_record.append({"Horizontal_Distance": horizontal_dist[j], "Height": height, "UAV_Send_Interval": usi, "MCS": curr_MCS, 
            ...
            if not next_mcs_rel_check:
                # Try next MCS
                if len(next_MCS_indexes) > 0:
                    curr_MCS_index = next_MCS_indexes.pop(0) 
                    curr_MCS = mcs_indexes[curr_MCS_index]
...
def run_mdp(height, usi, max_hdist_df, hdist_step_size, dl_model_path, ul_model_path, vid_model_path, reliability_th, save_path):
    ...
    if max_hdist > 0: # Exclude test cases with 0 max hdist 
        ...
        save_file = os.path.join(save_path, 'height-{}_usi-{}_predictions.csv'.format(height, usi))
        mdp_nn(dl_model, ul_model, vid_model, height, usi, reliability_th=reliability_th, horizontal_dist=hdist, save_file=save_file)

"""
Date: 07/06/2025
Desc: To get latency of each horizontal step in the MDP process
NOTE: Based on implementation in fanet_mdp_adaptation.ipynb
"""
...
def mdp_calibrated(dl_model_no_act, ul_model_no_act, vid_model_no_act, dl_T, ul_T, vid_T, reliability_th=0.99, 
    ...
    for j in tqdm(range(0, len(horizontal_dist))): # Traverse the h_dist
        ...
        # Record the latency for this horizontal distance step
        end_time = time.time()
        latency_record.append(end_time - start_time)
    ...
    # print(np.mean(latency_record))

    return mdp_actions, latency_record
...
def mdp_write_actions_2_xml(mdp_actions, nodes, uav_speed, xml_file_path,
    ...
    scenario_script = BeautifulSoup(ET.tostring(scenario_script.getroot(), 'utf-8'))
    with open(xml_file_path, 'w') as f:
        f.write(scenario_script.prettify())

""" Load NN Models """
dl_model = tf.keras.models.load_model("/media/research-student/KingstonSSD/FANET_Dataset/Dataset_NP100000_DJISpark/nn_ckpts/model_Downlink.round-1_split-9_0.2027.h5", compile=False)
...
ul_model = tf.keras.models.load_model("/media/research-student/KingstonSSD/FANET_Dataset/Dataset_NP100000_DJISpark/nn_ckpts/model_Uplink.round-1_split-9_0.1363.h5", compile=False)
...
vid_model = tf.keras.models.load_model("/media/research-student/KingstonSSD/FANET_Dataset/Dataset_NP100000_DJISpark/nn_ckpts/model_Video.round-1_split-9_0.2720.h5", compile=False)
...
cases = ["90", "99", "999"]
uav_speeds = [6, 16, 26]
reliability_th = {"90": 0.90, "99": 0.99, "999": 0.999}
calibration_t = {
    "90": {
...
heights_dict = {
    "90": [75,75,75,75,105,105,105,105,135,135,135,135,165,165,165,165,195,195,195,195,225,225,225,225,255,255,255,255,285,285,285,285],
    "99": [75,75,75,75,105,105,105,105,135,135,135,135,165,165,165,165,195,195,195,195,225,225,225,225,255,255,255,255,285,285,285,285],
    "999": [75,75,75,75,105,105,105,105,135,135,135,135,165,165,165,165,195,195,195,195,225,225,225,225,255,255,255,285,285,285]
}
usi_dict = {
    "90": [10,20,66.7,100,10,20,66.7,100,10,20,66.7,100,10,20,66.7,100,10,20,66.7,100,10,20,66.7,100,10,20,66.7,100,10,20,66.7,100],
    "99": [10,20,66.7,100,10,20,66.7,100,10,20,66.7,100,10,20,66.7,100,10,20,66.7,100,10,20,66.7,100,10,20,66.7,100,10,20,66.7,100],
    "999": [10,20,66.7,100,10,20,66.7,100,10,20,66.7,100,10,20,66.7,100,10,20,66.7,100,10,20,66.7,100,20,66.7,100,20,66.7,100],
}

""" MDP for calibrated NN """
latency = []
for uav_speed in uav_speeds:
    for case in cases:
        max_hdist_df = pd.read_csv("/home/research-student/omnet-fanet/data-processing-scripts/mdp_max_hdist/MDP_MCS_Max_HDist_fm_Sim_{}.csv".format(case))
        # Calibration T for this case
        dl_T = calibration_t[case]["dl_T"]
        ul_T = calibration_t[case]["ul_T"]
        vid_T = calibration_t[case]["vid_T"]
        # List of heights and USIs for this case
        heights = heights_dict[case]
        usi_list = usi_dict[case]
        for height, usi in zip(heights, usi_list):
            print("Case: {}, UAV Speed:{}, Height: {}, USI: {}".format(case, uav_speed, height, usi))
            ...
            if pd.isna(max_hdist): # For case test cases with 0 max hdist
                continue
            ...
            mdp_actions, latency_record = mdp_calibrated(dl_model_no_act, ul_model_no_act, vid_model_no_act, dl_T, ul_T, vid_T, reliability_th=reliability_th[case], 
                                         horizontal_dist=hdist, heights=[height], uav_send_ints=[usi], mcs_indexes=mcs_list, 
                                         init_height=height, init_usi=usi, init_mcs_index=init_mcs_index, save_file=None, end_at_max=False)
            latency += latency_record
            scenario_script_file = "/home/research-student/omnet-fanet/data-processing-scripts/journal_2_scripts/scenario_scripts_test/rel_th_{}/height-{}_usi-{}_uavspeed-{}_scenario_script.xml".format(case, height, usi, uav_speed)
            mdp_write_actions_2_xml(mdp_actions, nodes, uav_speed, scenario_script_file, adapt_usi=False, adapt_mcs=True)

print(f"Length of latency list: {len(latency)}")
print(f"Mean: {np.mean(latency)}, Min: {np.min(latency)}, Max: {np.max(latency)}, Std Dev: {np.std(latency)}")
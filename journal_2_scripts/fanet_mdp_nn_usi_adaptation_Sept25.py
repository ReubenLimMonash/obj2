...
def mdp_nn(dl_model, ul_model, vid_model, height, mcs, reliability_th=0.99, horizontal_dist = None, save_file=None):
    '''
    ...
    # init_usi_dl_results = dl_model.predict(inputs, verbose=0)
    # init_usi_dl_rel = [prob[0] for prob in init_usi_dl_results]
    # init_usi_ul_results = ul_model.predict(inputs, verbose=0)
    # init_usi_ul_rel = [prob[0] for prob in init_usi_ul_results]
    # init_usi_vid_results = vid_model.predict(inputs, verbose=0)
    # init_usi_vid_rel = [prob[0] for prob in init_usi_vid_results]
    ...
    for j in range(1, len(horizontal_dist)): # Traverse the h_dist
        ...
        while (next_dl_rel < reliability_th) or (next_ul_rel < reliability_th) or (next_vid_rel < reliability_th):
            ...
            mean_sinr = 2*(10*math.log10(m)-min_mean_sinr)/(max_mean_sinr-min_mean_sinr) - 1
            std_dev_sinr = 2*(10*math.log10(s)-min_std_dev_sinr)/(max_std_dev_sinr-min_std_dev_sinr) - 1
            next_dl_rel = dl_model.predict([[mean_sinr, std_dev_sinr, uav_send_int_norm[curr_usi], norm_MCS(mcs)]], verbose=0)[0][0]
            next_ul_rel = ul_model.predict([[mean_sinr, std_dev_sinr, uav_send_int_norm[curr_usi], norm_MCS(mcs)]], verbose=0)[0][0]
            next_vid_rel = vid_model.predict([[mean_sinr, std_dev_sinr, uav_send_int_norm[curr_usi], norm_MCS(mcs)]], verbose=0)[0][0]
...
def run_mdp(height, mcs, max_hdist_df, hdist_step_size, dl_model_path, ul_model_path, vid_model_path, reliability_th, save_path):
    ...
    if max_hdist > 0:  # Exclude test cases with 0 max hdist 
        ...
        mdp_nn(dl_model, ul_model, vid_model, height, mcs, reliability_th=reliability_th, horizontal_dist=hdist, save_file=save_file)

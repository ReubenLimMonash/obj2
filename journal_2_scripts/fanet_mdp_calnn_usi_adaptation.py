...
def run_mdp(uav_speed, height, mcs, init_usi, usi_list, max_hdist_df, hdist_step_size, nodes, 
            dl_model_path, ul_model_path, vid_model_path, dl_T, ul_T, vid_T, reliability_th, scenario_script_path):
    # Load models
    ...
    if (not pd.isna(max_hdist)) & (max_hdist > 0): # Exclude test cases with 0 max hdist
        ...
        mdp_actions = mdp_calibrated(dl_model, ul_model, vid_model, dl_T, ul_T, vid_T, reliability_th=reliability_th,
...
if __name__ == "__main__":
    ...
    SCENARIO_SCRIPT_PATH = "/home/research-student/omnet-fanet/scenario_scripts_usi_adaptation/scenario_scripts_1m_calibrated_4_specificity_90_v2"
    ...
    # MAX_HDIST_DF_PATH = "/home/clow0003/Reuben_ws/mdp_max_hdist/MDP_USI_Max_HDist_fm_Sim_999.csv"
    # SCENARIO_SCRIPT_PATH = "/home/clow0003/Reuben_ws/scenario_scripts_usi_adaptation/scenario_scripts_1m_calibrated_4_specificity_999_v2"
    RELIABILITY_TH = 0.9
    ...
    with Pool(NUM_WORKERS) as pool:
        pool.starmap(run_mdp, zip(*zip(*var_params), repeat(INIT_USI), repeat(USI_LIST), repeat(max_hdist_df), repeat(HDIST_STEP_SIZE), repeat(NODES), 
                    repeat(DL_NN_PATH), repeat(UL_NN_PATH), repeat(VID_NN_PATH), repeat(DL_T), repeat(UL_T), repeat(VID_T), 
                    repeat(RELIABILITY_TH), repeat(SCENARIO_SCRIPT_PATH)))
...
def process_scenario_v4(scenario_path, save_path, max_hdist_df, num_processes, 
    ...
    with Pool(num_processes) as pool:
        for result in pool.starmap(process_run, zip(run_number, repeat(run_file_path), repeat(uav_speed), repeat(max_hdist), 
                                                    repeat(dl_delay_threshold), repeat(ul_delay_threshold),repeat(vid_delay_threshold))):
...
if __name__ == "__main__":
    ...
    '''
    # MAX_HDIST_FILE = "/home/wlau0003/Reuben_ws/waypoint_max_hdist/Waypoint_Max_HDist_fm_Sim_50.csv"
    # max_hdist_df = pd.read_csv(MAX_HDIST_FILE)
    max_hdist_df = None # Put none if no need to limit by max hdist (Maybe max hdist already applied in simulation)

    sim_root_paths = ["/home/rlim0005/FANET_Dataset/Dataset_NP100000_DJISpark/max_hdist"]
    save_paths = ["/home/rlim0005/FANET_Dataset/Dataset_NP100000_DJISpark/max_hdist_processed"]

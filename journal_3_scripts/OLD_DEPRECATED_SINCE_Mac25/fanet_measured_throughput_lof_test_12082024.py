# Date: 14/08/2024
# Desc: To test LOF accuracy in no interference / UAV interference / MANET interference
...
if __name__ == "__main__":
    ...
    ''' Define Paths Here'''
    LOF_MODEL_PATH = "/media/research-student/KingstonSSD/lof_models"
    ...
    for scenario in tqdm(scenarios_no_int):
        ...
        # Load LOF Model and Robust Scaler
        lof_model_dl = load(open(os.path.join(LOF_MODEL_PATH, "lof_Downlink_USI-{}_MCS-{}.pkl".format(usi, mcs_index)), 'rb'))
        lof_model_ul = load(open(os.path.join(LOF_MODEL_PATH, "lof_Uplink_USI-{}_MCS-{}.pkl".format(usi, mcs_index)), 'rb'))
        lof_model_vid = load(open(os.path.join(LOF_MODEL_PATH, "lof_Video_USI-{}_MCS-{}.pkl".format(usi, mcs_index)), 'rb'))
        ...
        for run_num in range(len(uplink_throughput_files)):
            ...
            y_pred_no_int_lof = lof_model_dl.predict(X_test_no_int)
            y_pred_no_int_lof = (y_pred_no_int_lof + 1) // 2 # To convert the range from -1:1 to 0:1 (0 for outlier, 1 for inlier)
            dl_accuracy_score_lof = np.sum(y_pred_no_int_lof) / len(X_test_no_int) # Ground truth is all 1 for normal test data
            dl_num_samples = len(X_test_no_int)
            if dl_accuracy_score_lof < 1: 
                dl_int_detected = 1
            else:
                dl_int_detected = 0
            dl_run_results.append({"run_num": run_num, "dl_accuracy_score_lof": dl_accuracy_score_lof, "dl_num_samples": dl_num_samples, "dl_int_detected": dl_int_detected})
            # Uplink
            X_test_no_int = ul_throughput_df[["Mean_SINR", "Std_Dev_SINR", "Throughput_Norm"]].values
            y_pred_no_int_lof = lof_model_ul.predict(X_test_no_int)
            y_pred_no_int_lof = (y_pred_no_int_lof + 1) // 2 # To convert the range from -1:1 to 0:1 (0 for outlier, 1 for inlier)
            ul_accuracy_score_lof = np.sum(y_pred_no_int_lof) / len(X_test_no_int) # Ground truth is all 1 for normal test data
            ul_num_samples = len(X_test_no_int)
            if ul_accuracy_score_lof < 1: 
                ul_int_detected = 1
            else:
                ul_int_detected = 0
            ul_run_results.append({"run_num": run_num, "ul_accuracy_score_lof": ul_accuracy_score_lof, "ul_num_samples": ul_num_samples, "ul_int_detected": ul_int_detected})
            # Video
            X_test_no_int = vid_throughput_df[["Mean_SINR", "Std_Dev_SINR", "Throughput_Norm"]].values
            y_pred_no_int_lof = lof_model_vid.predict(X_test_no_int)
            y_pred_no_int_lof = (y_pred_no_int_lof + 1) // 2 # To convert the range from -1:1 to 0:1 (0 for outlier, 1 for inlier)
            vid_accuracy_score_lof = np.sum(y_pred_no_int_lof) / len(X_test_no_int) # Ground truth is all 1 for normal test data
            vid_num_samples = len(X_test_no_int)
            if vid_accuracy_score_lof < 1: 
                vid_int_detected = 1
            else:
                vid_int_detected = 0
            vid_run_results.append({"run_num": run_num, "vid_accuracy_score_lof": vid_accuracy_score_lof, "vid_num_samples": vid_num_samples, "vid_int_detected": vid_int_detected})
        ...
        dl_acc_avg = dl_run_results_df["dl_accuracy_score_lof"].mean()
        dl_acc_max = dl_run_results_df["dl_accuracy_score_lof"].max()
        dl_acc_min = dl_run_results_df["dl_accuracy_score_lof"].min()
        dl_acc_std_dev = dl_run_results_df["dl_accuracy_score_lof"].std()
        ...
        dl_run_results_df["dl_num_accuracy_samples"] = dl_run_results_df["dl_accuracy_score_lof"] * dl_run_results_df["dl_num_samples"]
        ...
        ul_acc_avg = ul_run_results_df["ul_accuracy_score_lof"].mean()
        ul_acc_max = ul_run_results_df["ul_accuracy_score_lof"].max()
        ul_acc_min = ul_run_results_df["ul_accuracy_score_lof"].min()
        ul_acc_std_dev = ul_run_results_df["ul_accuracy_score_lof"].std()
        ...
        ul_run_results_df["ul_num_accuracy_samples"] = ul_run_results_df["ul_accuracy_score_lof"] * ul_run_results_df["ul_num_samples"]
        ...
        vid_acc_avg = vid_run_results_df["vid_accuracy_score_lof"].mean()
        vid_acc_max = vid_run_results_df["vid_accuracy_score_lof"].max()
        vid_acc_min = vid_run_results_df["vid_accuracy_score_lof"].min()
        vid_acc_std_dev = vid_run_results_df["vid_accuracy_score_lof"].std()
        ...
        vid_run_results_df["vid_num_accuracy_samples"] = vid_run_results_df["vid_accuracy_score_lof"] * vid_run_results_df["vid_num_samples"]
    ...
    scenario_results_df.to_csv(os.path.join(SAVE_PATH, "lof_no_int_results.csv"))
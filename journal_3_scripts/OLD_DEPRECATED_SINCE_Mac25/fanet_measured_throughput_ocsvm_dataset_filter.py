'''
Date: 06/08/2024
Desc: To filter OCSVM training data to filter out lowest 0.5% of data by LOF
'''
...
if __name__ == "__main__":
    ...
    SAVE_PATH = "/media/research-student/KingstonSSD/FANET_Dataset/DJISpark_Measured_Throughput_100000_Samples/using_sim_reliability/ocsvm_models"
    ...
    NUM_NEIGHBOR = 500 # For LOF, set to 0.5% of number of samples
    NUM_PROCS = 32 # Number of processes to use
    ...
    for usi, mcs in tqdm(list(product(uav_send_int, mcs_index))):
        ...
        context_crit_dist_df = crit_dist_df.loc[(crit_dist_df["USI"]==usi) & (crit_dist_df["MCS_Index"]==mcs)]
        # For each height, get the data up to the critical distance
        df = []
        for h, cd in zip(context_crit_dist_df["Height"].values, context_crit_dist_df["D_max"].values):
    ...
    for usi, mcs in list(product(uav_send_int, mcs_index)):
        ...
        if not throughput_df.empty:
            ...
            print("-------------- Filtering by LOF -------------")
            print("Fitting LOF for USI {}, MCS {}".format(usi, mcs))
            lof = LocalOutlierFactor(n_neighbors=NUM_NEIGHBOR)
            lof.fit_predict(X_train)
            print("Done")
            lof_samples = lof.negative_outlier_factor_ * -1 # Negating negative LOF to get LOF
            idx_sort = np.argsort(lof_samples) # Sort LOF samples ascending
            X_train = X_train[idx_sort] # Sort X_train by LOF in ascending order, samples at end are more "outliers"
            inlier_split = math.floor(OCSVM_TARGET_SCORE * len(X_train)) # 99.5% of data to be inliers
            X_inlier = X_train[:inlier_split]
            X_outlier = X_train[inlier_split:]
            np.save(os.path.join(SAVE_PATH, "X_inlier_Downlink_USI-{}_MCS-{}.npy".format(usi, mcs)), X_inlier)
            np.save(os.path.join(SAVE_PATH, "X_outlier_Downlink_USI-{}_MCS-{}.npy".format(usi, mcs)), X_outlier)
    ...
    for usi, mcs in list(product(uav_send_int, mcs_index)):
        ...
        if not throughput_df.empty:
            ...
            X_inlier = X_train[:inlier_split]
            X_outlier = X_train[inlier_split:]
            np.save(os.path.join(SAVE_PATH, "X_inlier_Uplink_USI-{}_MCS-{}.npy".format(usi, mcs)), X_inlier)
            np.save(os.path.join(SAVE_PATH, "X_outlier_Uplink_USI-{}_MCS-{}.npy".format(usi, mcs)), X_outlier)
    ...
    for usi, mcs in list(product(uav_send_int, mcs_index)):
        ...
        if not throughput_df.empty:
            ...
            X_inlier = X_train[:inlier_split]
            X_outlier = X_train[inlier_split:]
            np.save(os.path.join(SAVE_PATH, "X_inlier_Video_USI-{}_MCS-{}.npy".format(usi, mcs)), X_inlier)
            np.save(os.path.join(SAVE_PATH, "X_outlier_Video_USI-{}_MCS-{}.npy".format(usi, mcs)), X_outlier)
...
Desc: Pandas data processing of FANET dataset with multiprocessing.
      ...
      Purpose: For processing throughput dataset (waypoint mode with rebroadcasting). [If we ever need]
'''
...
def process_run(run_number, file_path, save_path, scenario, height, uav_sending_interval, mcs_index, uav_speed, max_hdist,
    ...
    if max_hdist != None:
        ...
        uavs_br_df = uavs_br_df.loc[uavs_br_df["TxTime"] <= max_time]
        uavs_pd_df = uavs_pd_df.loc[uavs_pd_df["RxTime"] <= max_time] # Filtering by RxTime instead because TxTime is 0
        if not gcs_br_df.empty:
            gcs_br_df = gcs_br_df.loc[gcs_br_df["TxTime"] <= max_time]
    ...
    for i in range(1,len(rx_df_list)):
        ...
        if not uav_rx_df.empty:
            ...
            uav_throughput_df = process_throughput_sliding_window_time_broadcast(uav_rx_df_reliable, gcs_tx_df, dl_slot_size, dl_stride, max_time, dl_delay_threshold, uav_speed)
    ...
    if not gcs_rx_df.empty:
        ...
        gcs_rx_df_reliable = gcs_rx_df.loc[(gcs_rx_df["Delay"] <= ul_delay_threshold)]
...
if __name__ == "__main__":
    ...
    sim_root_paths = ["/home/clow0003/Reuben_ws/FANET_Dataset/DJISpark_Measured_Throughput_100000_Samples/data_waypoint"]
    save_paths = ["/home/clow0003/Reuben_ws/FANET_Dataset/DJISpark_Measured_Throughput_100000_Samples/data_waypoint_processed"]
    ...
    for sim_root_path, save_path in zip(sim_root_paths, save_paths):
        ...
        for scenario_path in scenario_list:
            process_scenario_broadcast(scenario_path, save_path, max_hdist_df, num_processes, dl_stride, ul_stride, dl_slot_size, ul_slot_size, dl_delay_threshold, ul_delay_threshold)

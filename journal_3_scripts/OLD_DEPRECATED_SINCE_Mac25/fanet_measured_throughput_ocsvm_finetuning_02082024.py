...
if __name__ == "__main__":
    ...
    for usi, mcs in list(product(uav_send_int, mcs_index)):
        ...
        if os.path.exists(inlier_npy) and os.path.exists(outlier_npy):
            ...
            dump(clf, open(os.path.join(SAVE_PATH, "ocsvm_{}_USI-{}_MCS-{}.pkl".format(link, usi, mcs)), 'wb'))
        else:
    ...
    for usi, mcs in list(product(uav_send_int, mcs_index)):
        ...
        if os.path.exists(inlier_npy) and os.path.exists(outlier_npy):
            ...
            dump(clf.best_estimator_, open(os.path.join(SAVE_PATH, "ocsvm_{}_USI-{}_MCS-{}.pkl".format(link, usi, mcs)), 'wb'))
        else:
            ocsvm_best_params_list.append({"USI": usi, "MCS": mcs, "Link": "UL", "Kernel": np.nan, "Gamma": np.nan, "Nu": np.nan})
            print("Empty X_Train - Link: {}, USI: {}, MCS: {}".format(link, usi, mcs))
    ocsvm_best_params_df = pd.DataFrame(ocsvm_best_params_list)

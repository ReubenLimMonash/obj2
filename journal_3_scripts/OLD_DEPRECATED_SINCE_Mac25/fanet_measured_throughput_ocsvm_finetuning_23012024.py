'''
Date: 23/01/2024
Desc: To finetune OCSVM models for measured throughput anomaly classification.
        Trains a new OCSVM model for different USI and MCS.
        Model Inputs: Mean and Std Dev of SINR (from height and hdist), measured throughput
        Model Output: 1 (No anomaly) and -1 (Anomaly)
        Uses Grid Search to fine tune Nu and Gamma parameters
'''
...
def normalize_throughput_robust_scaler(throughput_df, save_path):
    '''
    save_path is the path to save the robust scaler
    '''
    throughput = np.array(throughput_df["Throughput"].values)
    robust_scaler = RobustScaler().fit(throughput.reshape(-1,1))
    # Scale the throughput data in throughput_df
    throughput_df["Throughput"] = robust_scaler.transform(throughput.reshape(-1,1))
    # Save the robust scaler transformer
    dump(robust_scaler, open(save_path, 'wb'))
...
if __name__ == "__main__":
    ...
    DATASET_PATH = "/home/research-student/FANET_Dataset/DJISpark_Measured_Throughput_10000Samples/using_sim_reliability/data_ocsvm_train_processed" 
    SAVE_PATH = "/media/research-student/One Touch/FANET_Dataset/DJISpark_Measured_Throughput_100000Samples/using_sim_reliability/ocsvm_models"
    CRIT_DIST_FILE = "/media/research-student/One Touch/FANET_Dataset/DJISpark_Measured_Throughput_100000Samples/using_sim_reliability/ocsvm_models/Critical_Distances_Simulated_Reliability_v2.csv" # Set to "" to calc critical distances for each scenario
    ROBUST_SCALER_PATH = "" # Set to "" to generate from dataset
    # DL_MODEL_PATH = "/home/wlau0003/Reuben_ws/FANET_Dataset/Dataset_NP10000_DJISpark/nn_checkpoints/djispark_nnv4_wobn_dl/model.010-0.2039.h5"
    # UL_MODEL_PATH = "/home/wlau0003/Reuben_ws/FANET_Dataset/Dataset_NP10000_DJISpark/nn_checkpoints/djispark_nnv4_wobn_ul/model.010-0.1202.h5"
    # VID_MODEL_PATH = "/home/wlau0003/Reuben_ws/FANET_Dataset/Dataset_NP10000_DJISpark/nn_checkpoints/djispark_nnv4_wobn_vid/model.010-0.2581.h5"
    # DATASET_PATH = "/home/wlau0003/Reuben_ws/FANET_Dataset/DJISpark_Measured_Throughput_10000Samples/data_processed" 
    # SAVE_PATH = "/home/wlau0003/Reuben_ws/FANET_Dataset/DJISpark_Measured_Throughput_10000Samples/ocsvm_models_NP10000"
    # CRIT_DIST_FILE = "/home/wlau0003/Reuben_ws/FANET_Dataset/DJISpark_Measured_Throughput_10000Samples/ocsvm_models/Critical_Distances.csv" # Set to "" to calc critical distances for each scenario
    '''Define Finetuning Parameters Here'''
    NU = [1e-7, 5e-7, 1e-6, 5e-6, 1e-5, 5e-5, 1e-4, 5e-4, 1e-3, 5e-3, 1e-2, 5e-2]
    GAMMA = np.linspace(0.1,1,10).tolist() + np.linspace(2,20,10).tolist()
    OCSVM_TARGET_SCORE = 0.995 # Accuracy score to aim for 
    NUM_SAMPLES = 100000 # Number of samples to use. If less than this number of samples available for a scenario, max available samples will be used.
    NUM_PROCS = 24 # Number of processes to use
    SEED = 0 # Seed for pandas sample

    pandarallel.initialize(nb_workers=NUM_PROCS, progress_bar=False)
    '''Create the save directories if it doesn't exist'''
    if not os.path.isdir(SAVE_PATH): # For saving OCSVM models and RobustScaler model
        os.mkdir(SAVE_PATH)
    ...
    if CRIT_DIST_FILE == "":
        ...
        crit_dist_df.to_csv(os.path.join(SAVE_PATH, "Critical_Distances.csv"), index=False)
    ...
    dl_throughput_df_list = [] # To store the filtered DFs for different scenarios
    for usi, mcs in tqdm(list(product(uav_send_int, mcs_index))):
        ...
        if len(df) > 0: # If data exist for this case, process it and append to throughput_df_list
            ...
            throughput_df = throughput_df.sample(np.min([NUM_SAMPLES, len(throughput_df)]), replace=False, random_state=SEED) # Use only up to NUM_SAMPLES to train model
            if ROBUST_SCALER_PATH == "":
                throughput_df = normalize_throughput_robust_scaler(throughput_df, os.path.join(SAVE_PATH, "robust_scaler_Downlink_USI-{}_MCS-{}.pkl".format(usi, mcs)))
            else:
                robust_scaler = load(open(os.path.join(ROBUST_SCALER_PATH, "robust_scaler_Downlink_USI-{}_MCS-{}.pkl".format(usi, mcs)), 'rb'))
                throughput = np.array(throughput_df["Throughput"].values)
                throughput_df["Throughput"] = robust_scaler.transform(throughput.reshape(-1,1))
            ...
            throughput_df = normalize_data(throughput_df, columns=["Mean_SINR", "Std_Dev_SINR",], link="Downlink")
            dl_throughput_df_list.append(throughput_df)
        else: # Else append empty DF
            dl_throughput_df_list.append(pd.DataFrame())
    del dl_throughput_df
    gc.collect()

    # # UPLINK
    print("### UPLINK ###")
    ul_throughput_df = get_measured_throughput(DATASET_PATH, "Uplink")
    ul_throughput_df_list = [] # To store the filtered DFs for different scenarios
    for usi, mcs in tqdm(list(product(uav_send_int, mcs_index))):
        # Filter by USI and MCS
        ...
        if len(df) > 0: # If data exist for this case, process it and append to throughput_df_list
            ...
            throughput_df = throughput_df.sample(np.min([NUM_SAMPLES, len(throughput_df)]), replace=False, random_state=SEED) # Use only up to NUM_SAMPLES to train model
            if ROBUST_SCALER_PATH == "":
                throughput_df = normalize_throughput_robust_scaler(throughput_df, os.path.join(SAVE_PATH, "robust_scaler_Uplink_USI-{}_MCS-{}.pkl".format(usi, mcs)))
            else:
                robust_scaler = load(open(os.path.join(ROBUST_SCALER_PATH, "robust_scaler_Uplink_USI-{}_MCS-{}.pkl".format(usi, mcs)), 'rb'))
                throughput = np.array(throughput_df["Throughput"].values)
                throughput_df["Throughput"] = robust_scaler.transform(throughput.reshape(-1,1))
            throughput_df[['Mean_SINR',"Std_Dev_SINR"]] = throughput_df.parallel_apply(lambda row: sinr_lognormal_approx(row['Horizontal_Distance'],row['Height']),axis=1,result_type='expand')
            throughput_df = normalize_data(throughput_df, columns=["Mean_SINR", "Std_Dev_SINR",], link="Uplink")
            ul_throughput_df_list.append(throughput_df)
        else: # Else append empty DF
            ul_throughput_df_list.append(pd.DataFrame())
    del ul_throughput_df
    gc.collect()
       
    # VIDEO
    print("### VIDEO ###")
    vid_throughput_df = get_measured_throughput(DATASET_PATH, "Video")
    vid_throughput_df_list = [] # To store the filtered DFs for different scenarios
    for usi, mcs in tqdm(list(product(uav_send_int, mcs_index))):
        # Filter by USI and MCS
        ...
        if len(df) > 0: # If data exist for this case, process it and append to throughput_df_list
            ...
            throughput_df = throughput_df.sample(np.min([NUM_SAMPLES, len(throughput_df)]), replace=False, random_state=SEED) # Use only up to NUM_SAMPLES to train model
            if ROBUST_SCALER_PATH == "":
                throughput_df = normalize_throughput_robust_scaler(throughput_df, os.path.join(SAVE_PATH, "robust_scaler_Video_USI-{}_MCS-{}.pkl".format(usi, mcs)))
            else:
                robust_scaler = load(open(os.path.join(ROBUST_SCALER_PATH, "robust_scaler_Video_USI-{}_MCS-{}.pkl".format(usi, mcs)), 'rb'))
                throughput = np.array(throughput_df["Throughput"].values)
                throughput_df["Throughput"] = robust_scaler.transform(throughput.reshape(-1,1))
            throughput_df[['Mean_SINR',"Std_Dev_SINR"]] = throughput_df.parallel_apply(lambda row: sinr_lognormal_approx(row['Horizontal_Distance'],row['Height']),axis=1,result_type='expand')
            throughput_df = normalize_data(throughput_df, columns=["Mean_SINR", "Std_Dev_SINR",], link="Video")
            vid_throughput_df_list.append(throughput_df)
        else: # Else append empty DF
            vid_throughput_df_list.append(pd.DataFrame())
    del vid_throughput_df
    gc.collect()
            
    '''Training The OCSVM Models'''
    print("-------------- TRAINING OCSVM MODELS -------------")
    parameters = {'gamma': GAMMA, 'nu': NU, 'kernel': ['rbf']}

    # DOWNLINK
    print("### DOWNLINK ###")
    ocsvm_best_params_list = [] # For storing the best params found with Grid Search
    counter = 0
    for usi, mcs in list(product(uav_send_int, mcs_index)):
        throughput_df = dl_throughput_df_list[counter] # Get the corresponding dataset for this context (USI & MCS)
        if not throughput_df.empty:
            X_train = throughput_df[["Mean_SINR", "Std_Dev_SINR", "Throughput"]].values
            ocsvm_model = OneClassSVM()
            my_scorer = make_scorer(custom_loss_score, greater_is_better=False, accuracy_goal=OCSVM_TARGET_SCORE)
            clf = GridSearchCV(estimator=ocsvm_model, 
                            param_grid=parameters,
                            cv=5,
                            scoring=my_scorer,
                            n_jobs=NUM_PROCS, # -1 to use all procs
                            verbose=3) 
            clf.fit(X_train, np.ones(len(X_train)))
            ocsvm_best_params_list.append({"USI": usi, "MCS": mcs, "Link": "DL", "Kernel": clf.best_params_["kernel"], "Gamma": clf.best_params_["gamma"], "Nu": clf.best_params_["nu"]})
            dump(clf, open(os.path.join(SAVE_PATH, "ocsvm_Downlink_USI-{}_MCS-{}.pkl".format(usi, mcs)), 'wb'))
        else:
            ocsvm_best_params_list.append({"USI": usi, "MCS": mcs, "Link": "DL", "Kernel": np.nan, "Gamma": np.nan, "Nu": np.nan})
            print("Empty X_Train - Link: Downlink, USI: {}, MCS: {}".format(usi, mcs))
        counter += 1
    ocsvm_best_params_df = pd.DataFrame(ocsvm_best_params_list)
    ocsvm_best_params_df.to_csv(os.path.join(SAVE_PATH, "OCSVM_Best_Params_Downlink.csv"), index=False)
    ...
    counter = 0
    for usi, mcs in list(product(uav_send_int, mcs_index)):
        throughput_df = ul_throughput_df_list[counter] # Get the corresponding dataset for this context (USI & MCS)
        if not throughput_df.empty:
            X_train = throughput_df[["Mean_SINR", "Std_Dev_SINR", "Throughput"]].values
            ocsvm_model = OneClassSVM()
            my_scorer = make_scorer(custom_loss_score, greater_is_better=False, accuracy_goal=OCSVM_TARGET_SCORE)
            clf = GridSearchCV(estimator=ocsvm_model, 
                            ...
                            verbose=3) 
            clf.fit(X_train, np.ones(len(X_train)))
            ocsvm_best_params_list.append({"USI": usi, "MCS": mcs, "Link": "UL", "Kernel": clf.best_params_["kernel"], "Gamma": clf.best_params_["gamma"], "Nu": clf.best_params_["nu"]})
            dump(clf.best_estimator_, open(os.path.join(SAVE_PATH, "ocsvm_Uplink_USI-{}_MCS-{}.pkl".format(usi, mcs)), 'wb'))
        else:
            ocsvm_best_params_list.append({"USI": usi, "MCS": mcs, "Link": "UL", "Kernel": np.nan, "Gamma": np.nan, "Nu": np.nan})
            print("Empty X_Train - Link: Uplink, USI: {}, MCS: {}".format(usi, mcs))
        counter += 1
    ocsvm_best_params_df = pd.DataFrame(ocsvm_best_params_list)
    ocsvm_best_params_df.to_csv(os.path.join(SAVE_PATH, "OCSVM_Best_Params_Uplink.csv"), index=False)
    ...
    counter = 0
    for usi, mcs in list(product(uav_send_int, mcs_index)):
        throughput_df = vid_throughput_df_list[counter] # Get the corresponding dataset for this context (USI & MCS)
        if not throughput_df.empty:
            X_train = throughput_df[["Mean_SINR", "Std_Dev_SINR", "Throughput"]].values
            ocsvm_model = OneClassSVM()
            my_scorer = make_scorer(custom_loss_score, greater_is_better=False, accuracy_goal=OCSVM_TARGET_SCORE)
            clf = GridSearchCV(estimator=ocsvm_model, 
                            ...
                            verbose=3) 
            clf.fit(X_train, np.ones(len(X_train)))
            ocsvm_best_params_list.append({"USI": usi, "MCS": mcs, "Link": "VID", "Kernel": clf.best_params_["kernel"], "Gamma": clf.best_params_["gamma"], "Nu": clf.best_params_["nu"]})
            dump(clf.best_estimator_, open(os.path.join(SAVE_PATH, "ocsvm_Video_USI-{}_MCS-{}.pkl".format(usi, mcs)), 'wb'))
        else:
            ocsvm_best_params_list.append({"USI": usi, "MCS": mcs, "Link": "VID", "Kernel": np.nan, "Gamma": np.nan, "Nu": np.nan})
            print("Empty X_Train - Link: Video, USI: {}, MCS: {}".format(usi, mcs))
        counter += 1

    ocsvm_best_params_df = pd.DataFrame(ocsvm_best_params_list)
    ocsvm_best_params_df.to_csv(os.path.join(SAVE_PATH, "OCSVM_Best_Params_Video.csv"), index=False)
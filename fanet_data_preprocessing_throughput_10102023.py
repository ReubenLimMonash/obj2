...
def process_scenario_v4(scenario_path, save_path, dl_stride, ul_stride, dl_slot_size, ul_slot_size, dl_delay_threshold, ul_delay_threshold, mode='timeSlot', vid_stride=1, vid_slot_size=1, vid_delay_threshold=0, GX_Offset=0):
    ...
    for run in runs:
        ...
        max_time = np.nanmax([gcs_tx_df["TxTime"].max(), uavs_rx_df["RxTime"].max(), gcs_pd_df["RxTime"].max()])
        uavs_rx_df["Horizontal_Distance"] = uavs_rx_df["RxTime"] * uav_speed
        gcs_pd_df["Horizontal_Distance"] = gcs_pd_df["RxTime"] * uav_speed
        dl_throughput_df = process_throughput_sliding_window_time_v2(uavs_rx_df, gcs_pd_df, dl_slot_size, dl_stride, max_time, dl_delay_threshold)
        dl_throughput_df['Height'] = height
        dl_throughput_df['UAV_Sending_Interval'] = uav_sending_interval
        dl_throughput_df['MCS_Index'] = mcs_index
        dl_throughput_df['UAV_Speed'] = uav_speed
        dl_throughput_df_list.append(dl_throughput_df)
    ...
    dl_throughput_df.to_csv(os.path.join(save_path, scenario, "Downlink_Throughput.csv"), index=False)
...
if __name__ == "__main__":
    ...
    '''
    sim_root_path = "/media/research-student/One Touch/FANET Datasets/DJISpark_Measured_Throughput_10000Samples/data_waypoint"
    save_path = "/media/research-student/One Touch/FANET Datasets/DJISpark_Measured_Throughput_10000Samples/data_waypoint_processed_dl-5secWinSize100msecStride_ul-1secWinSize100msecStride"

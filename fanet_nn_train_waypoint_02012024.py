...
Desc: To train a NN classifier to predict FANET reliability and failure modes under waypoint mode
Modified: Adapted fanet_nn_train_12092023.py for waypoint mode. UAV Sending Interval not used as input to model
'''
...
def generate_reliability_train_test_dataset(dataset_details_df, test_split=0.2):
    ...
    for row in tqdm(dataset_details_df.itertuples()):
        ...
        if num_reliable > 1:
            reliable_packets = pd.DataFrame({"Mean_SINR": mean_sinr, "Std_Dev_SINR": std_dev_sinr, "MCS": mcs, "Packet_State": "Reliable"}, index=[0])
            ...
            reliable_packets_train = pd.DataFrame({"Mean_SINR": mean_sinr, "Std_Dev_SINR": std_dev_sinr, "MCS": mcs, "Packet_State": "Reliable"}, index=[0])
        ...
        if num_delay_excd > 1:
            delay_excd_packets = pd.DataFrame({"Mean_SINR": mean_sinr, "Std_Dev_SINR": std_dev_sinr, "MCS": mcs, "Packet_State": "Delay_Exceeded"}, index=[0])
            ...
            delay_excd_packets_train = pd.DataFrame({"Mean_SINR": mean_sinr, "Std_Dev_SINR": std_dev_sinr, "MCS": mcs, "Packet_State": "Delay_Exceeded"}, index=[0])
        ...
        if num_q_overflow > 1:
            q_overflow_packets = pd.DataFrame({"Mean_SINR": mean_sinr, "Std_Dev_SINR": std_dev_sinr, "MCS": mcs, "Packet_State": "QUEUE_OVERFLOW"}, index=[0])
            ...
            q_overflow_packets_train = pd.DataFrame({"Mean_SINR": mean_sinr, "Std_Dev_SINR": std_dev_sinr, "MCS": mcs, "Packet_State": "QUEUE_OVERFLOW"}, index=[0])
        ...
        if num_incr_rcvd > 1:
            incr_rcvd_packets = pd.DataFrame({"Mean_SINR": mean_sinr, "Std_Dev_SINR": std_dev_sinr, "MCS": mcs, "Packet_State": "RETRY_LIMIT_REACHED"}, index=[0])
            ...
            incr_rcvd_packets_train = pd.DataFrame({"Mean_SINR": mean_sinr, "Std_Dev_SINR": std_dev_sinr, "MCS": mcs, "Packet_State": "RETRY_LIMIT_REACHED"}, index=[0])
...
if __name__ == "__main__":
    ...
    CHECKPOINT_FILEPATH = '/home/rlim0005/nn_checkpoints/djispark_nnv4_wobn_waypoint_dl'
    DATASET_PATH = "/home/rlim0005/FANET_Dataset/Dataset_NP10000_DJISpark/DJI_Spark_Waypoint_Downlink_Reliability.csv"
    # CHECKPOINT_FILEPATH = '/home/clow0003/Reuben_ws/nn_checkpoints/djispark_nnv4_wobn_dl'
    # DATASET_PATH = "/home/clow0003/Reuben_ws/FANET_Dataset/Dataset_NP10000_DJISpark/DJI_Spark_Downlink_Reliability.csv"
    ...
    dataset_details_df = normalize_data(dataset_details_df, columns=["Mean_SINR", "Std_Dev_SINR", "MCS"], save_details_path=CHECKPOINT_FILEPATH) 
    ...
    X_train = df_train[["Mean_SINR", "Std_Dev_SINR", "MCS"]].values
    X_test = df_test[["Mean_SINR", "Std_Dev_SINR", "MCS"]].values
    ...
    packet_state_train = to_categorical(packet_state_train, num_classes=4)
    packet_state_test = to_categorical(packet_state_test, num_classes=4)
